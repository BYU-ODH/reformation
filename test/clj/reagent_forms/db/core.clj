...
(defstate ^:dynamic *db*
  :start {:datasource (hik/make-datasource (-> env :reagent-forms :db))}
...
;; CRUD
(defn dbc! [table-key entrymap] (jdbc/insert! *db* table-key entrymap))
(defn dbr [s] (map (partial transform-keys csk/->kebab-case-keyword) (jdbc/query *db* s)))

;; -*- eval: (rainbow-mode) -*-
(ns reagent-forms.styles.comments
  (:require [garden.def :refer [defstylesheet defstyles defkeyframes]]
            ...
            [reagent-forms.styles.definitions :as defs]))
  ...
  (let [new-comment-width (px 400)
    ...
    [:body
     [:.new-reply
      ...
      [:.reply-to-text
       {:border-style "solid"
        ...
        :padding (em 1)
        :margin-bottom (em 1)}]
      [:textarea {:dreagent-formslay "block"
                  :padding (em 0.5)
                  :width "100%"}]
      [:div.controls
       [:a
        {:dreagent-formslay "inline-block"
         :padding (em 0.5)
         :font-weight 600
         :margin (em 1)}]
     ...
     [:div.comments
      ...
      [:.comment
       {:padding (px 5)
        ...
        :text-align "left"}
       [:.badge ]
       [:.date :.author {:dreagent-formslay "inline-block"}]
     ...
     [:div.new-comment
      [:.comment-type
       {:float "left"}
       [:.tab {:dreagent-formslay "inline-block"
               :margin (px 5)
               :margin-bottom (px 0)

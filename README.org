...
** Overview
Reformation is a library for building web forms.
...
The main entry point for the library is `render-application, whic takes as its first argument a form specification---`easy-form` in the example above.

A form specification is just a vector of pairs.  The first member is a key, the second is a map describing the particular element.


#+BEGIN_SRC

[:example-radio {:type :radio
                 :options [{:content "first choice"}
...
Reformation ships with some pre-made form elements, but you can specify your own.  

#+BEGIN_SRC

[:example-custom {:type :custom
                  :fn (fn [fn-map {:keys [label on-change]}]
		        [:label {:for label}[:input {:on-change on-change 


#+END_SRC


Simply set `:type` to `:custom`, and provide a render function, and away you go!

The render function is just like any Reagent component function---it returns hiccup.  

Where it gets a bit more involved is the arguments it *receives*.

`[{:keys [READ UPDATE valpath] opt-map}]`

`READ` and `UPDATE` are both functions; `(READ valpath)` will return the value at the path associated with the element, and `(UPDATE valpath f)` works similarly to `update-in`.  

`opt-map` is simply a modified version of the form specification you've already passed in.  The one notable difference is that it includes `on-change`, which is simply

#+BEGIN_SRC
(defn on-change [event]
   (UPDATE valpath) (.. event -target -value))
#+END_SRC
